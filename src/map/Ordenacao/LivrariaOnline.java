...
public class LivrariaOnline {
    ...
    public void removerLivro(String titulo) {
        ...
        for (String l : livrosRemover) {
            livros.remove(l);
        }
    }

    public Map<String, Livro> exibirLivrosOrdenadosPorPreco() {
        List<Map.Entry<String, Livro>> livroPorPreco = new ArrayList<>(livros.entrySet());

        Collections.sort(livroPorPreco, new ComparatorPorPreco());

        Map<String, Livro> livroOrdenadosPorPreco = new LinkedHashMap<>();

        for (Map.Entry<String, Livro> entry : livroPorPreco) {
            livroOrdenadosPorPreco.put(entry.getKey(), entry.getValue());
        }
        return livroOrdenadosPorPreco;
    }
